...
const PODIUM_PRODUCT_CONFIG = {
  1: {
    medal: "🥇",
    card: "bg-gradient-to-b from-amber-50 to-yellow-100 border-yellow-300/60 shadow-[0_8px_20px_-4px_rgba(234,179,8,0.28)]",
  ...
  2: {
    medal: "🥈",
    card: "bg-gradient-to-b from-slate-50 to-slate-100 border-slate-200/80 shadow-[0_4px_12px_-4px_rgba(0,0,0,0.10)]",
  ...
  3: {
    medal: "🥉",
    card: "bg-gradient-to-b from-orange-50 to-amber-100 border-amber-200/60 shadow-[0_4px_12px_-4px_rgba(217,119,6,0.15)]",
...
function ProductPodiumCard({
  item,
  rank,
}: {
  item: { id: string; label: string; amount: number; imageUrl: string | null };
  rank: 1 | 2 | 3;
}) {
  const c = PODIUM_PRODUCT_CONFIG[rank];
  return (
    <div className={`flex flex-col items-center gap-1.5 rounded-2xl border px-2 py-3 ${c.card}`}>
      ...
      <p className={`font-black text-[#000666] leading-none tabular-nums ${c.amtSize}`}>
        {fmtMoney(item.amount)}
      </p>
    </div>
  );
    ...
    <section className="rounded-2xl bg-white p-5 shadow-[0_4px_20px_rgba(27,27,33,0.04),0_12px_40px_rgba(27,27,33,0.08)] sm:p-6">
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-3">
          {/* Podium — rank 1 on top centered, rank 2 & 3 below */}
          {top3[0] && (
            <div className="flex justify-center">
              <div className="w-[55%] min-w-[120px] max-w-[200px]">
                <ProductPodiumCard item={top3[0]} rank={1} />
              </div>
            </div>
          )}
          {top3.length > 1 && (
            <div className="flex gap-2">
              <div className="min-w-0 flex-1">
                {top3[1] && <ProductPodiumCard item={top3[1]} rank={2} />}
              </div>
              <div className="min-w-0 flex-1">
                {top3[2] && <ProductPodiumCard item={top3[2]} rank={3} />}
              </div>
            </div>
          )}
          ...
          {rest.length > 0 && (
            <div className="overflow-hidden rounded-xl border border-slate-100">
              {rest.map((item, i) => (
                <div
                  key={item.id}
                  className={`flex items-center gap-3 px-3 py-2.5 ${i < rest.length - 1 ? "border-b border-slate-100" : ""}`}
                >
                  <span className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#e9ecff] text-[10px] font-black text-[#000666]">
                    {i + 4}
                  </span>
                  ...
                  <p className="min-w-0 flex-1 truncate text-xs font-semibold text-[#1b1b21]">{item.label}</p>
                  <p className="shrink-0 text-xs font-bold tabular-nums text-[#000666]">{fmtMoney(item.amount)}</p>
                </div>
              ))}
            </div>
          )}
