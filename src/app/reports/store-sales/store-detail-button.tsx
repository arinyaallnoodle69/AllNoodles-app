...
export function StoreDetailButton({
  ...
  return (
    <>
      <button
        onClick={handleOpen}
        className="flex shrink-0 items-center gap-1 whitespace-nowrap rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-[#4A148C] hover:text-[#4A148C] active:scale-95"
      >
        ดูเพิ่มเติม
          ...
          <div className="flex h-[92dvh] w-full flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:h-auto sm:max-h-[85dvh] sm:max-w-2xl sm:rounded-2xl">
            ...
            <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
              <div className="min-w-0">
                ...
                <h2 className="mt-0.5 truncate text-base font-bold text-[#4A148C]">{customerName}</h2>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              {isPending ? (
                <div className="flex flex-col items-center justify-center gap-3 py-16 text-slate-400">
                  <Loader2 className="h-8 w-8 animate-spin text-[#4A148C]" strokeWidth={2} />
                    ...
                    return (
                      <div key={row.productId} className="flex items-center gap-3 px-5 py-3.5">
                        ...
                        <div className="shrink-0 text-right">
                          <p className="text-sm font-black text-[#4A148C] tabular-nums">
              ...
              <div className="shrink-0 border-t border-slate-100 bg-slate-50/60 px-5 py-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="text-right">
                    <span className="font-black text-[#4A148C]">{fmtMoney(totalRevenue)}</span>
                    <span
