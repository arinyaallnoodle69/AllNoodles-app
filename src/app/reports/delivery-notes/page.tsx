  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-[0_4px_20px_rgba(27,27,33,0.05)] transition-shadow hover:shadow-[0_12px_40px_rgba(27,27,33,0.09)] sm:p-5">
      <div className="mb-3 flex items-center justify-center gap-2.5 text-center">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-[#082A63] sm:h-9 sm:w-9">
          <Icon className="h-4.5 w-4.5" strokeWidth={2} />
        </div>
        <p className="text-sm font-semibold text-slate-500 sm:text-base">{label}</p>
      </div>
      <p className="text-center text-2xl font-black tracking-tight text-[#082A63] sm:text-3xl">{value}</p>
    </div>
  );
}
...
function DeliveryNoteRows({ row }: { row: DeliveryNoteReportRow }) {
  return (
    <>
      <tr className="border-t border-[#082A63]/12 bg-slate-50 text-slate-700">
        <td colSpan={8} className="px-4 py-2">
          <div className={`flex flex-wrap items-center gap-x-8 gap-y-2 ${styles.noteMetaWrap}`}>
            <p className={`flex min-w-0 items-center gap-2 text-sm font-semibold text-slate-700 ${styles.noteMetaItem}`}>
              <span className="shrink-0 text-[#082A63]">
            ...
            <p className={`flex min-w-0 items-center gap-2 text-sm font-semibold text-slate-700 ${styles.noteMetaItem}`}>
              ...
              <span className={`font-mono text-base font-bold text-[#082A63] ${styles.noteMetaValue}`}>{row.deliveryNumber}</span>
            </p>
          </div>
        </td>
      </tr>
      {row.lines.map((line) => (
        <tr key={line.id} className="bg-white">
          ...
          <td className="border-l border-[#082A63]/10 px-4 py-2">
          ...
          <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-semibold text-slate-700 whitespace-nowrap">{fmt(line.quantityDelivered)}</td>
          <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm text-slate-500 whitespace-nowrap">{line.saleUnitLabel}</td>
          <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm text-slate-600 whitespace-nowrap">{fmtMoney(line.lineCost)}</td>
          <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-semibold text-[#082A63] whitespace-nowrap">{fmtMoney(line.lineTotal)}</td>
          <td className={`border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-semibold whitespace-nowrap ${line.profit >= 0 ? "text-emerald-600" : "text-red-500"}`}>
            {fmtMoney(line.profit)}
          </td>
          <td className={`border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-semibold whitespace-nowrap ${line.lineTotal > 0 && line.profit >= 0 ? "text-emerald-600" : "text-slate-500"}`}>
      ...
      <tr className="bg-slate-50/80">
        ...
        <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-bold text-slate-700 whitespace-nowrap">{fmtMoney(row.totalCost)}</td>
        <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-black text-[#082A63] whitespace-nowrap">{fmtMoney(row.totalRevenue)}</td>
        <td className={`border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-black whitespace-nowrap ${row.netProfit >= 0 ? "text-emerald-600" : "text-red-500"}`}>
...
function DeliveryNoteMobileCard({ row }: { row: DeliveryNoteReportRow }) {
  return (
    <div className="bg-white px-3 py-3.5 sm:px-4">
      <div className="-mx-3 -mt-3.5 flex flex-col items-center gap-2 border-b border-white/15 bg-[#082A63] px-3 py-3 text-white sm:-mx-4 sm:px-4">
      ...
      <div className="mt-3 divide-y divide-[#082A63]/18">
        {row.lines.map((line) => (
          <div key={line.id} className="py-3 first:pt-0 last:pb-0">
            ...
            <div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2">
              <div className="bg-white px-3 py-2 text-center shadow-[0_8px_18px_rgba(27,27,33,0.1)]">
                ...
                <p className="mt-0.5 text-base font-black text-[#082A63]">{fmtMoneyCompact(line.lineTotal)}</p>
              </div>
      ...
      <div className="mt-3 border-t border-[#082A63]/10 pt-3">
        <div className="rounded-xl border border-[#082A63]/12 bg-white px-3 py-2.5 shadow-[0_6px_18px_rgba(27,27,33,0.08)]">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            ...
            <div className="bg-white px-2.5 py-2 text-center shadow-[0_8px_18px_rgba(27,27,33,0.1)]">
              ...
              <p className="mt-0.5 text-sm font-black text-[#082A63]">{fmtMoneyCompact(row.totalRevenue)}</p>
            </div>
      ...
      {pages.map((p, i) =>
        p === "..." ? (
          ...
          <Link
            key={p}
            href={`${baseUrl}&page=${p}`}
            className={`flex h-10 w-10 items-center justify-center rounded-xl text-base font-semibold transition ${p === page ? "bg-[#082A63] text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
              }`}
          >
            {p}
          </Link>
        ...
        <div className="mx-auto max-w-[1440px] px-4 py-6 sm:px-6 sm:py-8">
          <header className="mb-6 sm:mb-8">
            <nav className="mb-2 flex items-center gap-1 text-sm font-medium text-slate-400">
              ...
              <span className="font-semibold text-[#082A63]">รายงานใบจัดส่งรายวัน</span>
            </nav>
            <h1 className="text-2xl font-extrabold tracking-tight text-[#082A63] sm:text-3xl">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border-0 bg-slate-50 py-3.5 pl-9 pr-4 text-base text-slate-800 ring-1 ring-slate-200 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#082A63]/20"
                  />
                </div>
              </div>
              <div>
          ...
          <section className="rounded-2xl bg-white shadow-[0_4px_20px_rgba(27,27,33,0.05)]">
            <div className="hidden border-b border-slate-100 px-5 py-4 md:block sm:px-6 sm:py-5">
              <form method="GET" action="/reports/delivery-notes" className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end lg:flex-nowrap">
                <div className="w-full sm:min-w-[220px] sm:flex-1 lg:min-w-[280px] lg:max-w-[360px] lg:flex-[1.05]">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      defaultValue={keyword}
                      placeholder="เลขใบจัดส่ง หรือชื่อร้าน..."
                      className="w-full rounded-xl border-0 bg-slate-50 py-3 pl-9 pr-4 text-base text-slate-800 ring-1 ring-slate-200 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#082A63]/20"
                    />
                  </div>
                </div>
                ...
                <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row">
                  <button
                    type="submit"
                    className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#082A63] px-6 text-sm font-bold text-white transition hover:bg-[#103B82] active:scale-95 sm:w-auto"
                  >
                  ...
                  <Link
                    ...
                    className="flex h-10 w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-500 transition hover:border-[#082A63] hover:text-[#082A63] active:scale-95 sm:w-auto"
                  >
                    แสดงทั้งหมด
                  </Link>
                </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 sm:px-6 sm:py-5">
              <div>
                <h3 className="text-lg font-bold text-[#082A63] sm:text-xl">สรุปรายใบจัดส่งที่วางบิลแล้ว</h3>
                ...
                <div className="space-y-5 px-0 py-4 sm:px-5 sm:py-5 lg:block print:hidden">
                  {groupedRows.map((group) => {
                    ...
                    return (
                      <section
                        key={group.date}
                        className="-mx-0 rounded-none border-y border-[#082A63]/12 bg-white shadow-[0_4px_18px_rgba(27,27,33,0.05)] sm:mx-0 sm:rounded-2xl sm:border sm:border-[#082A63]/12"
                      >
                        <div className="lg:hidden">
                          <div className="border-b border-[#082A63]/12 bg-[#082A63] px-4 py-2.5">
                            <p className="text-sm font-semibold uppercase tracking-[0.12em] text-white/85">
                              วันที่ {isoToDisplay(group.date)}
                            </p>
                          </div>
                          <div className="divide-y divide-[#082A63]/16">
                            {group.items.map((row) => (
                        ...
                        <div className="hidden lg:block">
                          <div className="sticky top-0 z-20 border-b border-[#082A63]/12 bg-[#082A63] px-4 py-2.5 sm:px-5">
                            <p className="text-sm font-semibold uppercase tracking-[0.12em] text-white/85">
                              วันที่ {isoToDisplay(group.date)}
                            </p>
                          </div>
                          <div className="sticky top-[41px] z-10 border-b border-[#082A63]/12 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/90">
                            <div className="overflow-x-auto">
                              <table className="min-w-[980px] w-full border-collapse text-left">
                                ...
                                <thead>
                                  <tr className="bg-[#082A63]/[0.04]">
                                    <th className="px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.09em] text-slate-600 whitespace-nowrap">รหัสสินค้า</th>
                                    <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.09em] text-slate-600 whitespace-nowrap">สินค้า</th>
                                    <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600 whitespace-nowrap">จำนวน</th>
                                    <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600 whitespace-nowrap">หน่วย</th>
                                    <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600 whitespace-nowrap">ต้นทุน</th>
                                    <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600 whitespace-nowrap">ยอดขาย</th>
                                    <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600 whitespace-nowrap">กำไร</th>
                                    <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600 whitespace-nowrap">กำไร(%)</th>
                                  </tr>
                                </thead>
                              </table>
                          ...
                          <div className="overflow-x-auto">
                            <table className="min-w-[980px] w-full border-collapse text-left">
                              ...
                              <tbody className="divide-y divide-[#082A63]/10 bg-white">
                                {group.items.map((row) => (
                                ...
                                <tr className="bg-[#082A63]/[0.04]">
                                  ...
                                  <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-bold text-slate-700">
                                    {fmtMoney(totals.totalCost)}
                                  </td>
                                  <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-black text-[#082A63]">
                                    {fmtMoney(totals.totalRevenue)}
                                  </td>
                                  <td className={`border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-black ${totals.netProfit >= 0 ? "text-emerald-600" : "text-red-500"}`}>
                    ...
                    return pages.map((pageGroups, pageIdx) => (
                      <div key={pageIdx} data-print-page="true" className={`${styles.printArea} ${styles.printPage} print:break-after-page`}>
                                  ...
                                  <table className={`w-full border-collapse text-left ${styles.printTable}`}>
                                    ...
                                    <thead>
                                      <tr className="bg-[#082A63]/[0.04]">
                                        ...
                                        <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.09em] text-slate-600">สินค้า</th>
                                        <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600">จำนวน</th>
                                        <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600">หน่วย</th>
                                        <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600">ต้นทุน</th>
                                        <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600">ยอดขาย</th>
                                        <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600">กำไร</th>
                                        <th className="border-l border-[#082A63]/12 px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-[0.09em] text-slate-600">กำไร(%)</th>
                                      </tr>
                                    </thead>
                                    <tbody className="divide-y divide-[#082A63]/10 bg-white">
                                      ...
                                      <tr className="bg-[#082A63]/[0.04]">
                                        ...
                                        <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-bold">
                                          {fmtMoney(totals.totalCost)}
                                        </td>
                                        <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-black">
                                          {fmtMoney(totals.totalRevenue)}
                                        </td>
                                        <td className="border-l border-[#082A63]/10 px-4 py-2 text-center text-sm font-black">
                                          {fmtMoney(totals.netProfit)}
                        ...
                        {pageIdx === pages.length - 1 && (
                          <div className="mt-8 overflow-hidden rounded-xl border border-[#082A63]/20 bg-[#082A63]/[0.08]">
                            <div className="bg-[#082A63] px-6 py-3 text-white">
                              <h3 className="text-base font-bold">สรุปรวมทั้งหมด ({allRows.length} ใบจัดส่ง)</h3>
                            </div>
                            <div className="grid grid-cols-4 gap-0 divide-x divide-[#082A63]/10">
                              {[
                                ...
                                <div key={idx} className="p-4 text-center">
                                  ...
                                  <p className={`text-lg font-black ${item.color || "text-[#082A63]"}`}>{item.value}</p>
                                </div>
                              ))}
                            </div>
                          </div>
