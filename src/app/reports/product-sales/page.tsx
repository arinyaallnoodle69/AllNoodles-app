  ...
  return (
    <tr className="transition-colors hover:bg-slate-50/60">
      <td className="px-5 py-4 text-center">
        ...
        <span className={styles.printRankNumber}>{globalRank}</span>
      </td>
      ...
      <td
        className={`px-4 py-4 text-center tabular-nums whitespace-nowrap ${styles.printRevenueCell}`}
          ...
          <section className="overflow-hidden rounded-2xl bg-white shadow-[0_4px_20px_rgba(27,27,33,0.05)]">
            ...
            {rows.length === 0 ? (
              ...
              <>
                    ...
                    <tbody className="divide-y divide-[#003366]/24">
                      {rows.map((row, i) => (
                        <ProductRow
                          key={row.productId}
                          row={row}
                          globalRank={(page - 1) * pageSize + i + 1}
                        />
                      ))}
                ...
                <div id="report-print-area" className="hidden print:block">
                  {(() => {
                    ...
                    for (let i = 0; i < allRows.length; i += PAGE_SIZE) {
                      pages.push(allRows.slice(i, i + PAGE_SIZE));
                    }
                          ...
                          <tbody className="divide-y divide-[#003366]/24">
                            {pageRows.map((row, i) => (
                              <ProductRow
                                key={row.productId}
                                row={row}
                                globalRank={pageIdx * PAGE_SIZE + i + 1}
                              />
                            ))}
                ...
                {/* Mobile: cards (hidden on desktop, hidden on print) */}
