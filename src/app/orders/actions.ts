...
async function resolveProductSaleUnitId(
  ...
  if (saleUnitLabel) {
    const matched = (saleUnits as Array<{ id: string; unit_label: string }>).find(
      (u) => u.unit_label === saleUnitLabel,
    );
...
async function backfillOrderItemPrices(
  ...
  // Find orders for this customer that are not fulfilled or cancelled
  // IMPORTANT: Use .or() to handle NULL statuses correctly in Postgres
  const { data: customerOrders } = await admin
    .from("orders")
    .select("id")
    .eq("customer_id", customerId)
    .or("fulfillment_status.neq.complete,fulfillment_status.is.null")
  ...
  // Fetch items and match by ID or Label
  const { data: itemsToUpdate } = await admin
    .from("order_items")
    .select("id, order_id, quantity, product_id, product_sale_unit_id, sale_unit_label")
  ...
  const filteredItems = (
    ...
    (item) =>
      item.product_sale_unit_id === productSaleUnitId ||
      (saleUnitLabel && item.sale_unit_label === saleUnitLabel),
  );
  ...
  for (const item of filteredItems) {
    await admin
      ...
      .update({
        ...
        // Heal the ID if it was missing
        product_sale_unit_id: item.product_sale_unit_id || productSaleUnitId,
      })
      .eq("id", item.id);
  }

  const affectedOrderIds = [...new Set(filteredItems.map((item) => item.order_id))];
...
async function upsertStorePriceBatch(
  ...
  const resolvedItems = await Promise.all(
    normalizedItems.map(async (item) => {
      ...
      return {
        ...
        sale_unit_label: item.saleUnitLabel // useful for tracking
      };
    }),
  );
  ...
  await pricesTable.upsert(
    upsertRows.map((row) => ({
      ...
      product_sale_unit_id: row.product_sale_unit_id,
  ...
  await Promise.all(
    upsertRows.map((item) =>
      backfillOrderItemPrices(
        admin,
        customerId,
        item.product_sale_unit_id,
        item.sale_unit_label || null,
        item.sale_price
      ),
    ),
  );
...
export async function getStoreItemCostSnapshots(items: CostSnapshotInput[]) {
  ...
  return normalizedItems.map((item) => {
    ...
    // Try to find the sale unit by ID first, then by Label
    const saleUnit =
      (item.productSaleUnitId ? saleUnitById.get(item.productSaleUnitId) : null) ??
      (item.saleUnitLabel ? productSaleUnits.find(u => u.unit_label === item.saleUnitLabel) : null) ??
...
export async function getShortagesForDate(orderDate: string) {
  const session = await requireAppRole("admin");
  const { getGlobalShortageDetails } = await import("@/lib/orders/admin");
  return getGlobalShortageDetails(session.organizationId, orderDate);
}
