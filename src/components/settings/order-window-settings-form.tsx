...
function ModernToggle({
  ...
  return (
    <button
      ...
      onClick={onChange}
      disabled={loading}
      className={`relative inline-flex h-[52px] w-[104px] shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-4 focus:ring-[#082A63]/20 disabled:cursor-not-allowed disabled:opacity-70 ${
        checked ? "bg-[#082A63]" : "bg-slate-300"
      }`}
    >
      ...
      >
        {loading ? (
          <Loader2
            className={`h-5 w-5 animate-spin ${checked ? "text-[#082A63]" : "text-slate-400"}`}
          />
        ) : (
          <span className={`text-base font-bold ${checked ? "text-[#082A63]" : "text-slate-500"}`}>
            {checked ? "เปิด" : "ปิด"}
          </span>
        )}
      </span>
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="relative">
        <select
          ...
          className="block w-[88px] appearance-none rounded-[1rem] border-0 bg-slate-100 py-3 pl-5 pr-8 text-center text-xl font-bold text-[#082A63] shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-2 focus:ring-[#082A63]/30 sm:w-24 sm:pl-6 sm:text-2xl"
        >
          {HOURS.map((hour) => (
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-8">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-md transition-shadow hover:shadow-lg">
        <div className="border-b border-slate-100 bg-[#082A63]/[0.08] px-6 py-6 sm:px-8 sm:py-8">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[#082A63] text-white shadow-lg shadow-[#082A63]/20">
        ...
        <form action={formAction} className="px-6 py-8 sm:px-8 sm:py-10">
          ...
          <div className="mb-10 flex flex-col items-center justify-between gap-6 rounded-2xl border border-[#082A63]/10 bg-[#082A63]/[0.10] p-6 lg:flex-row lg:p-8">
            <div className="flex items-start gap-4">
              <div className="mt-1 flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-white text-[#082A63] shadow-sm">
          ...
          <div className="flex flex-col-reverse gap-4 sm:flex-row sm:justify-end">
            ...
            <button
              type="submit"
              disabled={!hasChanges || isPending}
              className="inline-flex items-center justify-center gap-3 rounded-2xl bg-[#082A63] px-10 py-4 text-lg font-bold text-white shadow-lg shadow-[#082A63]/20 transition hover:bg-[#103B82] disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-500 disabled:shadow-none"
            >
