    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        (blob) => {
          ...
          resolve(new File([blob], file.name.replace(/\.[^.]+$/, ".jpg"), { type: "image/png" }));
        },
        "image/jpeg",
        0.85,
      );
...
function ProductFormBody({
  ...
  const initialSkuValue = isEditing ? (editingProduct?.sku ?? "") : nextSku;
  const skuValue = initialSkuValue;
  ...
  }, [showSuccess, isEditing, onSubmitSuccess]);
  ...
  const hasUnsavedChanges = useMemo(() => {
    ...
    const initialSaleUnits = editingProduct.saleUnits.map((saleUnit) => ({
      baseUnitQuantity: Number(saleUnit.baseUnitQuantity),
      costMode: saleUnit.costMode,
      fixedCostPrice:
        saleUnit.costMode === "fixed"
          ? Number(saleUnit.fixedCostPrice ?? 0)
          : null,
      id: saleUnit.id,
      label: saleUnit.label,
      minOrderQty: Number(saleUnit.minOrderQty),
      stepOrderQty: saleUnit.stepOrderQty === null ? null : Number(saleUnit.stepOrderQty),
    }));

    const currentSaleUnits = saleUnits.map((saleUnit) => ({
      baseUnitQuantity: getParsedNumber(saleUnit.baseUnitQuantity),
      costMode: saleUnit.costMode,
      fixedCostPrice:
        saleUnit.costMode === "fixed"
          ? getParsedNumber(saleUnit.fixedCostPrice || "0")
          : null,
      id: saleUnit.id,
      label: saleUnit.label,
      minOrderQty: getParsedNumber(saleUnit.minOrderQty),
      stepOrderQty:
        saleUnit.orderPreset === "free"
          ? null
          : saleUnit.orderPreset === "integer"
            ? INTEGER_ORDER_PRESET_QTY
            : saleUnit.stepOrderQty.trim()
              ? getParsedNumber(saleUnit.stepOrderQty)
              : null,
    }));

    const sameBasicFields =
      basicFormValues.name === (editingProduct.name ?? "") &&
    ...
    const sameImages =
      keptExistingUrls.length === editingProduct.imageUrls.length &&
      keptExistingUrls.every((url, index) => url === editingProduct.imageUrls[index]) &&
      files.length === 0 &&
      !prioritizeNewImages;

    return !(sameBasicFields && sameSaleUnits && sameImages);
  }, [
    isEditing,
    editingProduct,
    basicFormValues,
    ...
    <>
      ...
      >
                    ...
                    <div className="flex gap-2 overflow-x-auto">
                      {galleryItems.map((preview, index) => (
                        <button
                          ...
                          className={`relative h-20 w-20 shrink-0 overflow-hidden rounded-xl border ${index === activeIndex ? "border-[#4A148C]" : "border-slate-200"
                            }`}
                        >
                          <Image
                            src={preview.url}
                          ...
                          />
                          {index === 0 ? (
                            <span className="absolute bottom-1 left-1 rounded-full bg-[#4A148C] px-2 py-0.5 text-[10px] font-bold text-white">
                              รูปหลัก
                            </span>
                          ) : null}
                          {preview.isExisting ? (
        ...
        <div className="flex shrink-0 items-center justify-end gap-3 border-t border-slate-100 bg-white px-5 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:px-6">
          ...
          >
            ...
            {isPending ? "กำลังบันทึก..." : isEditing ? (hasUnsavedChanges ? "บันทึกการแก้ไข" : "ยังไม่มีการเปลี่ยนแปลง") : "บันทึกสินค้า"}
          </button>
        </div>
      </form>
      ...
      {showSuccess ? (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 z-[80] w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl bg-white shadow-[0_20px_56px_rgba(4,53,106,0.28)] ring-1 ring-[#4A148C]/15">
            <div className="flex items-center gap-3 px-4 py-3.5">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#4A148C]/20">
                <CheckCircle2 className="h-5 w-5 text-[#4A148C]" strokeWidth={2} />
              </div>
          ...
          <div className="flex max-h-[96dvh] w-full max-w-2xl flex-col overflow-hidden rounded-[1.5rem] bg-slate-950 text-white shadow-[0_28px_80px_rgba(15,23,42,0.4)]">
            ...
            <div className="flex shrink-0 items-center justify-end gap-3 border-t border-white/10 bg-slate-950 px-5 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-[#4A148C] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#4A148C]"
              >
      ...
      >
        ...
        <div className="shrink-0 border-b border-slate-100 px-4 pb-3 pt-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            {hasNav ? (
              <button
                ...
                className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#4A148C] text-white shadow-md transition hover:bg-[#4A148C] disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none"
              >
          ...
          <div className="mt-2 flex items-center gap-2">
            {isEditing ? (
              <Package2 className="h-5 w-5 shrink-0 text-[#4A148C]" strokeWidth={2.2} />
            ) : (
