...
function CustomerCountButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={count === 0}
      className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-black text-[#082A63] shadow-sm transition hover:border-[#082A63]/35 hover:bg-[#082A63]/15 disabled:cursor-not-allowed disabled:border-slate-100 disabled:bg-slate-50 disabled:text-slate-400 disabled:shadow-none"
    >
  ...
  return (
    <div className="flex items-center gap-2">
      <Link
        ...
        className="inline-flex items-center gap-1 rounded-full border border-slate-200 px-3 py-1.5 text-xs font-medium text-[#1F2A44] transition hover:border-[#082A63]/30 hover:text-[#082A63]"
      >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {vehicle.licensePlate ? (
                            <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium text-[#1F2A44]">
                              <IdCard className="h-3.5 w-3.5 text-[#082A63]" strokeWidth={2.2} />
                              {vehicle.licensePlate}
                            </div>
                          ) : null}
                          ...
                          {vehicle.driverName ? (
                            <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium text-[#1F2A44]">
                              <UserRound className="h-3.5 w-3.5 text-[#082A63]" strokeWidth={2.2} />
                              {vehicle.driverName}
                            </div>
                          ) : null}
                ...
                <table className="min-w-full border-collapse text-left">
                  <thead>
                    <tr className="bg-slate-50">
                      <th className="px-6 py-4 text-xs font-semibold uppercase tracking-[0.16em] text-[#1F2A44]">
                        ชื่อรถ
                      </th>
                      <th className="px-6 py-4 text-xs font-semibold uppercase tracking-[0.16em] text-[#1F2A44]">
                        ทะเบียนรถ
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {vehicles.map((vehicle) => (
                      <tr key={vehicle.id}>
                        ...
                        <td className="px-6 py-4 text-sm text-[#1F2A44]">
                          {vehicle.licensePlate || <span className="text-[#1F2A44]">-</span>}
                        </td>
                        <td className="px-6 py-4 text-sm text-[#1F2A44]">
                          {vehicle.driverName || <span className="text-[#1F2A44]">-</span>}
                        </td>
                        <td className="px-6 py-4">
                          <CustomerCountButton
                            vehicle={vehicle}
            ...
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-5 sm:px-6">
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-black uppercase tracking-[0.14em] text-[#1F2A44]">
                    รายชื่อร้านค้า
                  </p>
                  <h3 className="mt-1 truncate text-xl font-black text-slate-950">
                    {selectedVehicle.name}
                  </h3>
                  <p className="mt-1 text-sm font-bold text-[#082A63]">
            ...
            <div className="overflow-y-auto px-5 py-5 sm:px-6">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {selectedVehicle.customers.map((customer) => (
                  ...
                  >
                    <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-[#082A63]">
                      <Store className="h-4.5 w-4.5" strokeWidth={2.3} />
                    </div>
                    <p className="text-xs font-black text-[#082A63]">{customer.code}</p>
