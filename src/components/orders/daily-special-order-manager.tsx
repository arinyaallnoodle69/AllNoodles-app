    ...
    <>
      ...
      {isOpen && typeof document !== "undefined" ? createPortal(
        <div className="fixed inset-0 z-[600] flex items-end justify-center bg-slate-950/45 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) setIsOpen(false); }}>
          <div className="flex h-[94dvh] w-full max-w-[100vw] flex-col overflow-hidden rounded-t-[28px] bg-[#FBF8FC] shadow-2xl sm:h-[88vh] sm:max-w-[1320px] sm:rounded-[30px]">
              ...
              <main className="flex min-h-0 min-w-0 flex-col overflow-hidden">
                <div className="shrink-0 space-y-3 border-b border-[#E1BEE7]/70 bg-white px-4 py-3 sm:px-6">
                  ...
                  <label className="relative block sm:hidden">
                    <Truck className="pointer-events-none absolute left-3.5 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-[#4A148C]" />
                    <select
                      value={vehicleId}
                      onChange={(event) => { setVehicleId(event.target.value); setDraft({}); }}
                      className="h-11 w-full appearance-none rounded-xl border border-[#CE93D8] bg-white pl-10 pr-10 text-sm font-black text-[#4A148C] outline-none focus:border-[#7B1FA2] focus:ring-2 focus:ring-[#EA80FC]/25"
                      aria-label="เลือกรถ"
                    >
                      {vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.name}</option>)}
                    </select>
                    <ChevronDown className="pointer-events-none absolute right-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[#4A148C]" />
                  </label>
                  ...
                  <label className="relative block"><Search className="absolute left-3.5 top-1/2 h-4.5 w-4.5 -translate-y-1/2 text-[#4A148C]/60" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ค้นหาชื่อหรือรหัสสินค้า" className="h-11 w-full rounded-xl border border-[#E1BEE7] bg-[#FBF8FC] pl-10 pr-4 text-sm font-bold text-slate-900 outline-none focus:border-[#AB47BC] focus:ring-2 focus:ring-[#EA80FC]/20" /></label>
                </div>
                ...
                <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-3 sm:px-6">
                  <div className="grid gap-2 xl:grid-cols-2">
                    {filteredProducts.map((product) => {
                      ...
                      return <article key={product.id} className={`grid min-w-0 grid-cols-[44px_minmax(0,1fr)] items-start gap-x-3 gap-y-2 rounded-2xl border bg-white px-3 py-3 transition sm:grid-cols-[44px_minmax(0,1fr)_112px] sm:items-center sm:gap-3 sm:p-2.5 ${selected ? type === "claim" ? "border-pink-400 shadow-[0_8px_20px_rgba(236,72,153,.10)]" : "border-[#9C27B0] shadow-[0_8px_20px_rgba(74,20,140,.10)]" : "border-slate-200"}`}>
                        <button type="button" onClick={() => updateDraft(product.id, selected ? 0 : 1)} className="relative row-span-2 h-11 w-11 self-start overflow-hidden bg-transparent sm:row-span-1 sm:self-center sm:rounded-xl sm:bg-slate-50 sm:ring-1 sm:ring-slate-200">
                          {product.imageUrl ? <Image src={product.imageUrl} alt={product.name} fill sizes="44px" className="object-contain sm:p-1" /> : <PackageCheck className="m-2.5 h-6 w-6 text-slate-300" />}
                          <span className={`absolute left-0.5 top-0.5 grid h-4 w-4 place-items-center rounded border text-white ${selected ? "border-[#4A148C] bg-[#4A148C]" : "border-slate-300 bg-white"}`}>{selected ? <Check className="h-3 w-3" /> : null}</span>
                        </button>
                        <button type="button" onClick={() => updateDraft(product.id, selected ? 0 : 1)} className="min-w-0 self-start pr-1 text-left sm:self-center sm:pr-0"><strong className="block max-h-10 overflow-hidden whitespace-normal break-words text-sm font-black leading-5 text-slate-900">{product.name}</strong><span className="mt-1 block text-[11px] font-bold leading-4 text-slate-500">{product.sku} · {product.unit}</span></button>
                        <div className="col-start-2 row-start-2 grid w-28 grid-cols-[32px_1fr_32px] justify-self-end overflow-hidden rounded-xl border border-slate-200 bg-slate-50 sm:col-start-3 sm:row-start-1 sm:w-auto sm:self-center"><button type="button" onClick={() => updateDraft(product.id, quantity - 1)} className="grid h-9 place-items-center text-slate-500 active:bg-slate-200"><Minus className="h-3.5 w-3.5" /></button><input inputMode="decimal" value={quantity || ""} onChange={(event) => updateDraft(product.id, Number(event.target.value))} placeholder="0" className="min-w-0 bg-white text-center text-sm font-black outline-none" /><button type="button" onClick={() => updateDraft(product.id, quantity + 1)} className="grid h-9 place-items-center text-[#4A148C] active:bg-[#F3E5F5]"><Plus className="h-3.5 w-3.5" /></button></div>
                      </article>;
                    })}
                  </div>
                </div>
            ...
            <div className="shrink-0 border-t border-[#E1BEE7] bg-white p-3 lg:hidden">
              <details className="group"><summary className="flex list-none items-center justify-between rounded-xl bg-[#F3E5F5] px-4 py-3 text-sm font-black text-[#4A148C]"><span>ตะกร้า {cart.length} รายการ</span><ChevronDown className="h-4 w-4 transition group-open:rotate-180" /></summary><div className="max-h-40 space-y-1 overflow-y-auto py-2">{cart.map((item) => <div key={cartKey(item)} className="grid grid-cols-[1fr_auto_auto] items-center gap-2 px-2 py-1"><span className="truncate text-xs font-bold">{vehicleById.get(item.vehicleId)?.name} · {typeMeta[item.type].label} · {productById.get(item.productId)?.name}</span><strong className="text-xs">{item.quantity}</strong><button type="button" onClick={() => removeFromCart(item)} className="p-1 text-rose-500"><Trash2 className="h-3.5 w-3.5" /></button></div>)}</div></details>
              {error ? <p className="mt-2 text-xs font-bold text-rose-600">{error}</p> : null}<button type="button" onClick={saveAll} disabled={isPending} className="mt-2 h-12 w-full rounded-2xl bg-[#4A148C] text-sm font-black text-white shadow-lg disabled:opacity-60">{isPending ? "กำลังบันทึก..." : "บันทึกทั้งหมด"}</button>
            </div>
          </div>
