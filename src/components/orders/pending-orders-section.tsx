...
function DeliveryModal({
  ...
  const deliveryProductStockStatus = useMemo(() => {
    ...
    for (const item of deliveryItems) {
      const selectedQty = parseFloat(qtys[item.orderItemId] ?? "0") || 0;
      ...
      pendingBaseByProduct.set(
        item.productId,
        (pendingBaseByProduct.get(item.productId) ?? 0) + item.remainingBaseQty,
      );
      availableBaseByProduct.set(
        item.productId,
        Math.max(availableBaseByProduct.get(item.productId) ?? 0, item.availableStock),
      );
    }
    ...
    for (const [productId, selectedBase] of selectedBaseByProduct.entries()) {
      ...
      if (selectedBase > availableBase + pendingBase + 0.000001) {
  ...
  const overStockItems: DeliveryItemData[] = deliveryItems.filter((item) => {
    const qty = parseFloat(qtys[item.orderItemId] ?? "0");
    return qty > 0 && deliveryProductStockStatus.overProducts.has(item.productId);
  });
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {deliveryItems.map((item) => {
                ...
                const isOver =
                  qtyNum > 0 && deliveryProductStockStatus.overProducts.has(item.productId);
...
function StoreDeliveryModal({
  ...
  const productStockStatus = useMemo(() => {
    ...
    for (const item of groupedItems) {
      const selectedQty = parseFloat(qtys[item.groupKey] ?? "0") || 0;
      const selectedBase = selectedQty * item.saleUnitRatio;
      if (selectedBase > 0) {
        selectedBaseByProduct.set(
          item.productId,
  ...
  const overStockGroups = groupedItems.filter((g) => {
    const qty = parseFloat(qtys[g.groupKey] ?? "0");
    return qty > 0 && productStockStatus.overProducts.has(g.productId);
  });
