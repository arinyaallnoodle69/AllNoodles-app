    ...
    >
      <div className="space-y-3 xl:grid xl:grid-cols-[minmax(0,1fr)_176px_176px_auto] xl:items-end xl:gap-3 xl:space-y-0">
        ...
        <div className="flex gap-2 xl:justify-end">
          <button
            type="submit"
            className="inline-flex flex-1 items-center justify-center rounded-xl bg-[#4A148C] px-5 py-3 text-base font-semibold text-white transition hover:bg-[#4A148C] xl:flex-none"
          >
            ค้นหา
          </button>
          {hasQuery && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4">
        <article className="flex flex-col items-center justify-center border-b border-r border-slate-200 px-4 py-3 text-center md:border-b-0">
          <div className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
            <Building2 className="h-3.5 w-3.5 text-[#4A148C]" strokeWidth={2.1} />
            <span>ร้านค้า</span>
          </div>
        ...
        <article className="flex flex-col items-center justify-center border-b border-slate-200 px-4 py-3 text-center md:border-b-0 md:border-r">
          <div className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
            <Package2 className="h-3.5 w-3.5 text-[#4A148C]" strokeWidth={2.1} />
            <span>รายการรวม</span>
          </div>
        ...
        <article className="flex flex-col items-center justify-center border-r border-slate-200 px-4 py-3 text-center">
          <div className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
            <FileText className="h-3.5 w-3.5 text-[#4A148C]" strokeWidth={2.1} />
        ...
        <article className="flex flex-col items-center justify-center px-4 py-3 text-center">
          <div className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
            <Truck className="h-3.5 w-3.5 text-[#4A148C]" strokeWidth={2.1} />
            <span>ยอดจัดส่ง</span>
          </div>
    ...
    return (
      <div className="flex items-center justify-center gap-2">
        ...
        <button
          type="button"
          onClick={startEdit}
          className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-slate-200 text-slate-400 hover:border-[#4A148C] hover:text-[#4A148C]"
      ...
      <div className="flex items-center gap-1.5">
        <div className="flex flex-col items-end gap-1">
          <input
            ...
            className="w-20 rounded-md border border-slate-300 px-2 py-1 text-right text-sm font-semibold focus:border-[#4A148C] focus:outline-none"
          />
          <select
            value={mode}
            onChange={(e) => setMode(e.target.value as DeliveryAdjustmentMode)}
            className="w-32 rounded-md border border-slate-300 bg-white px-2 py-1 text-xs text-slate-700 focus:border-[#4A148C] focus:outline-none"
          >
        ...
        <button
          ...
          onClick={saveEdit}
          disabled={isPending}
          className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-[#4A148C] text-white hover:bg-[#4A148C] disabled:opacity-50"
          aria-label="บันทึก"
        >
  ...
  return (
    <>
      <button
        type="button"
        onClick={openDialog}
        className="text-sm font-semibold text-[#4A148C] underline decoration-slate-300 underline-offset-2 transition hover:text-[#4A148C]"
      >
        แก้ไข
      </button>
          ...
          <div className="w-full max-w-sm overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            <div className="space-y-4 px-5 py-5">
              <label className="block">
                ...
                <input
                  ...
                  value={qty}
                  onChange={(e) => setQty(e.target.value)}
                  className="w-full rounded-2xl border border-slate-300 px-4 py-3 text-right text-lg font-semibold text-slate-900 outline-none transition focus:border-[#4A148C] focus:ring-2 focus:ring-[#4A148C]/10"
                />
              </label>
              ...
              <label className="block">
                ...
                <select
                  value={mode}
                  onChange={(e) => setMode(e.target.value as DeliveryAdjustmentMode)}
                  className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-base text-slate-800 outline-none transition focus:border-[#4A148C] focus:ring-2 focus:ring-[#4A148C]/10"
                >
                  <option value="return_to_stock">คืนสต็อก</option>
                  <option value="lost">ของหาย/เสียหาย</option>
                </select>
            ...
            <div className="flex gap-3 border-t border-slate-200 px-5 py-4">
              ...
              <button
                ...
                onClick={saveEdit}
                disabled={isPending}
                className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-[#4A148C] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#4A148C] disabled:opacity-50"
              >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      <div className="relative z-10 flex max-h-[95svh] w-full max-w-2xl flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:max-h-[90vh] sm:rounded-3xl">
        {/* Header */}
        <div className="flex shrink-0 items-start justify-between bg-[#4A148C] px-6 py-5">
          <div>
                ...
                <div className="flex items-center justify-between bg-slate-50 px-6 py-3">
                  {orderNumber && orderDate ? (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 font-mono text-sm font-bold text-[#4A148C] transition hover:text-[#4A148C]"
                    >
                      {orderNumber}
...
function DeliveryCustomerCard({
  ...
  return (
    <div className={`overflow-hidden rounded-2xl border-2 bg-white shadow-sm ${tone.accentClass}`}>
      {/* Card header */}
      <div className="bg-[#4A148C] px-4 py-3">
...
function DeliveryCustomerSection({
  ...
  return (
    <>
      ...
      <tr>
        <td colSpan={8} className="border-t-2 border-[#4A148C] bg-[#4A148C] px-4 py-2">
...
function DeliveryDayTable({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm md:rounded">
      ...
      <div className="z-20 flex items-center gap-x-3 border-b border-l-4 border-slate-200 border-l-[#4A148C] bg-slate-50 px-4 py-2.5 md:sticky md:top-0">
        <span className="text-sm font-bold text-[#4A148C]">{fmtDateLabel(date)}</span>
