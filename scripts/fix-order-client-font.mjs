...
// 1. Replace captureReceiptImage function to use html-to-image with font embedding
...
const captureReceiptImageReplacement = `const captureReceiptImage = useCallback(async (): Promise<{ blob: Blob; fileName: string } | null> => {
    ...
    try {
      ...
      cloneHost.style.cssText = [
        "position:fixed",
        "left:-10000px", // Use -10000px like in working report page
        "top:0",
...
console.log('Successfully added font embedding to order-client.tsx');
