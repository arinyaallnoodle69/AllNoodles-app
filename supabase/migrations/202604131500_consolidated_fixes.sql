-- Consolidated Fixes: Inventory, Stock Logic, and Store Visibility
-- Description: 
-- 1. Fixes inventory receipt to avoid unit constraint violations and handle ratios.
-- 2. Updates shortage logic to be date-specific (ignore past stale orders).
-- 3. Hides completed stores from the summary and updates indexing (1/4 -> 1/3).
-- 4. Adds auto-cleanup function for stale orders.
-- Date: 2026-04-13

BEGIN;

-- 1. FIX: Create/Update inventory receipt function
-- Handles unit ratios and avoids changing the base product unit
...
BEGIN
  ...
  LOOP
    ...
    -- IMPORTANT: We update stock and cost price but NEVER change the 'unit' column
...
-- 2. FIX: Date-specific shortage logic for items
...
AS $$
  WITH daily_total_demand AS (
    -- Calculate demand for THIS DATE ONLY to avoid "stale order" ghosting
    SELECT 
      oi.product_id,
  ...
  SELECT
    ...
    -- If total daily demand fits in stock, this specific order is fully deliverable
    CASE 
...
-- 3. FIX: Date-specific shortage summary and HIDE completed stores
