...
BEGIN
  ...
  SELECT
    ...
    dni.unit_price,
    dni.sale_unit_ratio,
    coalesce(dni.warehouse_id, dn.warehouse_id)
  INTO
    v_dn_id,
    v_dn_number,
    v_order_item_id,
  ...
  IF v_mode = 'return_to_stock' AND v_qty_base_delta > 0 THEN
    IF v_warehouse_id IS NULL THEN
      RAISE EXCEPTION 'ใบส่งของนี้ยังไม่ได้ผูกคลัง ไม่สามารถคืนสต็อคได้';
    END IF;

    PERFORM public.ensure_product_warehouse_stock(
      p_organization_id,
      v_product_id,
      v_warehouse_id
    );
    ...
    WHERE pws.organization_id = p_organization_id
      ...
      AND pws.warehouse_id = v_warehouse_id
    FOR UPDATE;

    IF v_stock_before IS NULL THEN
      RAISE EXCEPTION 'Warehouse stock not found for this product';
    END IF;
